...
@auth_user
def my_orders(request, id_user):
    get_id_order = request.GET.get('get_id_order')

    search_my_orders = Order_Items.objects.filter(id_client=id_user)
    if search_my_orders:
...
@auth_user
def orders_my_client(request, id_user):
    all_orders = Order_Items.objects.filter(authors_items__icontains=id_user).values()
    ...
    if 'get_id_order' in request.POST:
        ...
        get_order = Order_Items.objects.filter(authors_items__icontains=id_user, id=id_order).first()

        get_order.id_confirmed_sellers += [id_user]
        ...
        for i in all_orders:
            ...
            for item in Items.objects.filter(id__in=id_items, author_id_item__icontains=id_user).all():
                item.amount_item -= 1
                item.save()
...
def client_items(request, id_user, get_name_client, get_id_order):
    ...
    for number, item_ids in sort_data.items():  
        info_item = Items.objects.filter(id__in=item_ids, author_id_item=id_user).values()  
        show_order_item[number] = list(info_item)
